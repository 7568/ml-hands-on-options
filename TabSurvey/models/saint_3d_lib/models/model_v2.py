        ...
        for _ in range(depth):
            if self.style == 'colrow':
                self.layers.append(nn.ModuleList([
                    nn.ModuleList([nn.ModuleList([PreNorm(dim, Residual(Attention(dim, heads=1, dropout=attn_dropout))),
                                                  PreNorm(dim, Residual(FeedForward(dim, dropout=ff_dropout)))]) for i
                                   in range(5)]),
                    nn.ModuleList([nn.ModuleList([PreNorm(dim * nfeats // 5, Residual(
                        Attention(dim * nfeats // 5, heads=nfeats // 5, dropout=attn_dropout))),
                                                  PreNorm(dim * nfeats // 5, Residual(
                                                      FeedForward(dim * nfeats // 5, dropout=ff_dropout)))]) for i in
                                   range(5)]),
