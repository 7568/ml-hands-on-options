...
def no_hedge_in_training(clean_data):
    put_data, call_data = get_data('training', clean_data)
    no_hedge_result(put_data, call_data)


def no_hedge_in_validation(clean_data):
    put_data, call_data = get_data('validation', clean_data)
    no_hedge_result(put_data, call_data)


def no_hedge_in_test(clean_data):
    # put_results, call_results = get_test_data(clean_data)
    put_data, call_data = get_data('testing', clean_data)
    no_hedge_result(put_data, call_data)


def no_hedge_result(put_results, call_results):
    delta = 0
...
if __name__ == '__main__':
    ...
    no_hedge_in_validation(CLEAN_DATA)
