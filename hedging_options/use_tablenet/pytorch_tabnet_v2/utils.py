...
class OptionPriceDataset(Dataset):
    def __init__(self, data_path, one_day_data_numbers, use_features):
        """
        ...
        # 将0-1之间均匀的分成file_number+1份，断点的位置就有 file_number 个，他们的取值为:
        # all_points = 1 - np.cos(np.arange(1 / 430, 1, 1 / 430) * np.pi / 2)
        all_points = np.power(np.arange(1 / file_number, 1, 1 / file_number) * np.pi / 2, 1)
        self.data_length = all_points.shape[0]
        # 计算每个值占总和的比例，这就是训练的时候取数的比例，这样做是为了时间越往后的数据让他越多的被训练到
        all_rates = all_points / np.sum(all_points)
        # 让最小的数缩放成1，然后其他的数依次以相同的比例缩放
        all_rates = np.rint(all_rates / all_rates[0])  # 最终大概有9W多条数据
        all_index = np.array([])
    ...
    def __getitem__(self, index):
        file_index = self.all_files[self.all_data_index[index]]
        one_day_all_data = pd.read_parquet(f'{self.data_path}/{file_index}_data.parquet')
