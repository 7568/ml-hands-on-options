...
# This file contains functions that implement the linear regression methods.
...
def rolling_lin_reg2(
    ...
    groupnames = [str(x) for x in grouped.groups.keys()]
    tmp = pd.MultiIndex.from_product([groupnames, features], names=['Group', 'Coef'])
    df_coef, df_fit_std = [pd.DataFrame(columns=tmp) for _ in range(2)]
    if leverage:
        df_leve = pd.DataFrame(columns=groupnames)
    # else:
    #     df_leve = None
    ...
    for name, group in grouped:
        ...
        if leverage:
            var = fit_leverage(df_train)
            df_leve.loc[str(name)] = var

        # df_coef.loc[str(name)] = regs['regr'].coef_
        df_fit_std.loc[str(name)] = regs['std']
        deltas.loc[df_test.index] = predicted_linear_delta(regs['regr'], df_test, features)

    return {'delta': deltas.dropna(), 'df_coef': df_coef, 'df_leve': None,
            'df_fit_std': df_fit_std}
