...
def bs_formula_call(vol, S, K, tau, r):
    ...
    implied volatility.
    """
    if (np.sqrt(vol ** 2 * tau)) == 0:
        d1 = ((np.log(S / K) + (r + vol ** 2 / 2) * tau) / (np.sqrt(vol ** 2 * tau) + 1e-10))
    else:
...
def calc_d1(vol, S, K, tau, r):
    ...
    d1[~ix_int] = (np.log(S[~ix_int] / K[~ix_int]) + (r[~ix_int] + vol[~ix_int] ** 2 / 2) * tau[~ix_int]) / np.sqrt(
        vol[~ix_int] ** 2 * tau[~ix_int])
    d1[ix_int] = (np.log(S[ix_int] / K[ix_int]) + (r[ix_int] + vol[ix_int] ** 2 / 2) * tau[ix_int]) / np.sqrt(
        vol[ix_int] ** 2 * (tau[ix_int] + 1e-10))
...
def find_impvol(row):
    ...
    # print(f'computing one option impl vol time spent: {te - ts}')
    return vol
...
def df_apply(data_chunk):
    return data_chunk.apply(find_impvol, axis=1)


def calc_implvol(df):  # wrapper
    # df.to_csv('/home/liyu/data/hedging-option/calc_implvol_1.csv')
    # ts = datetime.datetime.now()
    # print('Start time (computing impl vol):', ts)
    cpu_num = cpu_count() - 95
    if cpu_num > len(df):
        cpu_num = len(df)
    print(cpu_num)
    data_chunks = chunks(df, cpu_num)
    with Pool(cpu_num) as p:
        r = p.map(df_apply, data_chunks)
        p.close()
        p.join()
    ...
    df.loc[:, 'implvol0'] = new_df
    # df.to_csv('/home/liyu/data/hedging-option/calc_implvol_2.csv')
    te = datetime.datetime.now()
...
if __name__ == '__main__':
    df_1 = pd.read_csv('/home/liyu/data/hedging-option/calc_implvol_1.csv', index_col=0, parse_dates=['date'])
    # df_2 = pd.read_csv('/home/liyu/data/hedging-option/calc_implvol_2.csv', index_col=0, parse_dates=['date'])
    # # calc_implvol(df)
    # df = df_2.reset_index()
    ...
    # print('finish==============')
    # _row = df_1.iloc[408]
    # df_1.loc[df_1['V0'] < 0, 'V0'] = 0
    calc_implvol(df_1)
    # _row['V0'] = 0
    # vol = find_impvol(_row)
    # print(vol)
...
def normalize_prices(
    ...
    df[cols_after] = df[cols].values / np.expand_dims(s_divisor / norm_factor, axis=1)
    return df
...
def append_1M_ATM_option(mc_one, paras):
    ...
    if underlying_model == 'BS':
        ...
        print(f'mc_atm.shape = {mc_atm.shape}')
