...
def choose_period_span(df, days):
    option_ids = df['optionid'].unique()
    for _id in option_ids:
        _options = df[df['optionid'] == _id]
        if (_options['date'].max() - _options['date'].min()).days < days:
            df = df[df['optionid'] != _id]
    return df


def choose_period_span_for_china_precess(param):
    df = param['df']
    days = param['days']
    option_ids = df['SecurityID'].unique()
    df_large = pd.DataFrame(columns=df.columns)
    df_small = pd.DataFrame(columns=df.columns)
    for _id in tqdm(option_ids, total=len(option_ids)):
        _options = df[df['SecurityID'] == _id]
        if _options.shape[0] < days:
            df_small = df_small.append(_options)
        else:
            df_large = df_large.append(_options)
    return [df_large, df_small]


def choose_period_span_for_china(df, days):
    cpu_num = cpu_count() - 56
    # cpu_num = 1
    if cpu_num > len(df):
        cpu_num = len(df)
    data_chunks = cm.chunks(df, cpu_num)
    param = []
    for data_chunk in data_chunks:
        ARGS_ = dict(days=days, df=data_chunk)
        param.append(ARGS_)
    with Pool(cpu_num) as p:
        r = p.map(choose_period_span_for_china_precess, param)
        p.close()
        p.join()
    print('run done!')
    ...
    for _r in r:
        df_large = df_large.append(_r[0])
        df_small = df_small.append(_r[1])
    return df_large, df_small
