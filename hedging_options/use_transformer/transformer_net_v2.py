...
class EncoderLayer(nn.Module):
    def __init__(self, input_dim, hid_dim, n_heads, dropout, device):
        ...
        self.time_dim_self_attention = MultiHeadAttentionLayer(15, hid_dim, n_heads, dropout, device)
        self.feature_dim_self_attention = MultiHeadAttentionLayer(input_dim, hid_dim, n_heads, dropout, device)
    ...
    def forward(self, src):
        ...
        # self attention
        # print(src.shape)
        _src, _ = self.time_dim_self_attention(torch.transpose(src, 1, 2))
        __src, __ = self.feature_dim_self_attention(src)

        # dropout, residual connection and layer norm
        src = self.self_attn_layer_norm(src + self.dropout(torch.transpose(_src, 1, 2) + __src))
